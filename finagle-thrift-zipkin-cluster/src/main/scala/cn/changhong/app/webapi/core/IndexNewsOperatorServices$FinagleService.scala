...
class IndexNewsOperatorServices$FinagleService(
  ...
  addFunction("deleteArtNews", { (iprot: TProtocol, seqid: Int) =>
    try {
      val args = deleteArtNews$args.decode(iprot)
      iprot.readMessageEnd()
      (try {
        iface.deleteArtNews(args.id)
      } catch {
        case e: Exception => Future.exception(e)
      }) flatMap { value: Int =>
        reply("deleteArtNews", seqid, deleteArtNews$result(success = Some(value)))
      } rescue {
        case e => Future.exception(e)
      }
    } catch {
      case e: TProtocolException => {
        iprot.readMessageEnd()
        exception("deleteArtNews", seqid, TApplicationException.PROTOCOL_ERROR, e.getMessage)
      }
      case e: Exception => Future.exception(e)
    }
  })
