...
trait Tables {
  ...
  case class AccessTokenRow(accessToken: String, refreshToken: String, userId: Long, clientId: String, createdAt: Int, expiresIn: Int)
  ...
  implicit def GetResultAccessTokenRow(implicit e0: GR[String], e1: GR[Long], e2: GR[Int]): GR[AccessTokenRow] = GR{
    prs => import prs._
    AccessTokenRow.tupled((<<[String], <<[String], <<[Long], <<[String], <<[Int], <<[Int]))
  }
