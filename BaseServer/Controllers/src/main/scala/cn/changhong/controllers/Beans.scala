 ...
 */
trait CHRequest
case class RequestBean[R](request:HttpRequest,path:String,params:R) extends CHRequest
trait CHResponse
case class ResponseContent[V](status:Int,content:Option[V]) extends CHResponse
object ResponseCode {
  // 请求成功
  val succeed = 0
  // 不匹配任何账户
